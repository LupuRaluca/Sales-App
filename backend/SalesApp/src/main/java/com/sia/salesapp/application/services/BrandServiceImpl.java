...
public class BrandServiceImpl implements BrandService {
    ...
    @Override public BrandResponse create(BrandRequest req) {
        if (repo.existsByNameIgnoreCase(req.name()))
            throw new IllegalArgumentException("Brand deja existent");
        Brand b = repo.save(Brand.builder().name(req.name()).description(req.description()).build());
        return new BrandResponse(b.getId(), b.getName(), b.getDescription());
    }
    @Override public BrandResponse update(Long id, BrandRequest req) {
    ...
    @Override public BrandResponse get(Long id) {
        Brand b = repo.findById(id).orElseThrow(() -> new EntityNotFoundException("Brand inexistent"));
        return new BrandResponse(b.getId(), b.getName(), b.getDescription());
    }
    @Override public List<BrandResponse> list() {
        return repo.findAll().stream().map(b -> new BrandResponse(b.getId(), b.getName(), b.getDescription())).toList();
    }
}
